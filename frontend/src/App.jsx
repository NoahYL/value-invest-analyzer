...
function App() {
      ...
      <>
        ...
        <div className="stock-card stock-header-card">
          <div className="stock-header">
            ...
            <span className={`stock-market ${data.market === "A股" ? "a-share" : "us-stock"}`}>
              {data.market}
            </span>
          </div>
        </div>
            ...
            <div className="stock-card">
              <div className="stock-details">
                ...
                <div className="detail-row">
                  ...
                  <span className="value business">{data.business || "暂无数据"}</span>
                </div>
              </div>

              {data.concepts && data.concepts.length > 0 && (
                <div className="section">
                  <h3 className="section-title">概念板块</h3>
                  <div className="tags">
                    {data.concepts.map((c) => (
                ...
                <div className="section">
                  ...
                  {mostProfitable && (
                    <div className="highlight-box">
                      ...
                      （毛利率 {mostProfitable.gross_margin}%，收入占比 {mostProfitable.ratio}%）
                    </div>
                  )}
                  <div className="revenue-table">
                    ...
                    {topRevenue.map((r) => (
                      <div key={r.name} className="rev-row">
                        ...
                        <span className="rev-cell num">{formatIncome(r.income)}</span>
                        <span className="rev-cell num">{r.ratio}%</span>
                        <span className={`rev-cell num ${r.gross_margin >= 30 ? "high-margin" : r.gross_margin >= 15 ? "mid-margin" : "low-margin"}`}>
                          {r.gross_margin}%
                        </span>
                        <span className="rev-cell bar-col">
                          <span className="bar" style={{ width: `${Math.min(r.ratio, 100)}%` }}></span>
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              {/* 估值分析 */}
              <div className="section">
                <h3 className="section-title">估值分析</h3>
        ...
        {/* 同行对比 - 全宽 */}
        <div className="stock-card full-width-card">
    ...
    <div className="stock-list">
      ...
      {items.map((item) => (
        ...
        >
          ...
          <span className={`stock-market ${item.market === "A股" ? "a-share" : "us-stock"}`}>
            {item.market}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="app">
      ...
      <p className="subtitle">输入股票代码或公司名称，查看公司基本信息</p>
